...
$services = CIBlockElement::GetList( //Получаем ID текущей услуги
    ...
    false,
    array(),
    array('ID', 'CODE', 'PROPERTY_NAME', 'PROPERTY_PROCENT', 'PROPERTY_CENA', 'PROPERTY_FOTO'));
...
while ($service = $services->GetNext() ):
    ...
    <main>
            ...
            <div class="container">
                <div class="path">
                    <ul>
                        ...
                        <li><a href="#"><?=$service['PROPERTY_NAME_VALUE']?></a></li>
                    </ul>
                </div>
            </div>
        </section>
                ...
                <div class="details-content">
                    <div class="details-block border border-b">
                        ...
                        <? $service_detail = CIBlockElement::GetList(
                            ...
                            false,
                            array(),
                            array('ID','CODE', 'SECTION_CODE', 'NAME', 'PROPERTY_DISCOUNT_NAME','PROPERTY_DISCOUNT_PRICE', 'PROPERTY_SCROLL_IN_ONE','PROPERTY_SERVICE_INCLUDES', 'PROPERTY_SCROLL_NAME', 'PROPERTY_SCROLL_NAME_EN','PROPERTY_SCROLL_MAX','PROPERTY_SCROLL_MIN','PROPERTY_LIST_NAME','PROPERTY_LIST_PRICE','PROPERTY_CHECK_NAME','PROPERTY_CHECK_PRICE','PROPERTY_TANK_LIST','PROPERTY_NAME1_TANK','PROPERTY_PRICE1_TANK','PROPERTY_PRICE3_TANK','PROPERTY_PRICE4_TANK','PROPERTY_PRICE5_TANK','PROPERTY_PRICE2_TANK','PROPERTY_NAME2_TANK','PROPERTY_NAME3_TANK','PROPERTY_NAME4_TANK','PROPERTY_NAME5_TANK', 'PROPERTY_MIN_MATERIAL_COUNT', 'PROPERTY_MIN_BASEPRICE_COUNT'));
                        ?>
                        ...
                        <form action="#" class="details-form form detail-usluga-ajax" data-params="<?=$currentID?>">
                            ...
                            <div class="form-content" >
                                ...
                                if(count($service_additional['PROPERTY_CHECK_NAME_VALUE']) || count($service_additional['PROPERTY_NAME1_TANK_VALUE'])): ?>
                                    <style>
                                        .label-checkbox
                                        {
                                            max-height: ;
                                ...
                                $lbzCount = count($service_additionalCNT['PROPERTY_TANK_LIST_VALUE']);

                                ?>
                                <? if (empty($service_additional['PROPERTY_TANK_LIST_VALUE'])){?>
                                <div class="details-form__block details-form__block-type details-form__block-range">
                                    <div class="details-form__block-title"><?=$service_additional['PROPERTY_SERVICE_INCLUDES_VALUE']?></div>
                                    <? if (!empty($service_additional['PROPERTY_LIST_NAME_VALUE'])):
                                    ...
                                    <div>
                                        ...
                                        <div class="details-form__block-filter">
                                            ...
                                            if ($service_additional['ID'] == '448'):?>
                                                ...
                                                <div class="details-form__block-title"><?=$service_additional['PROPERTY_LIST_NAME_VALUE'][0]?></div>
                                                <?php
                                                $basePrice = $items[0]['PROPERTY_LIST_PRICE_VALUE'][0];
                                                ?>
                                                <input class="base-id-input" type="hidden" name="USLUGI[BASENAME][ID]" value="0">
                                                <? for($i = 0; $i < count($service_additional['PROPERTY_LIST_NAME_VALUE']);$i++): ?>
                                                ...
                                                <div class="quick-filter border">
                                                    <div class="select">
                                                        <div class="select_checked quick-select_checked">
                                                            <span class="select-text"><?=$service_additional['PROPERTY_LIST_NAME_VALUE'][0]?></span>
                                                            <input type="hidden" >
                                                        </div>
                                                        <?php
                                                        $basePrice = $items[0]['PROPERTY_LIST_PRICE_VALUE'][0];
                                                        ...
                                                        <ul class="select-dropdown">
                                                            <input class="base-id-input" type="hidden" name="USLUGI[BASENAME][ID]" value="0">
                                                            <? for($i = 0; $i < count($service_additional['PROPERTY_LIST_NAME_VALUE']);$i++): ?>
                                                                <li  selector-id="<?=$i?>" data-id="<?=$items[$i]['ID']?>" class="select-dropdown__option quick-filter__option" data-filter="<?=$service_additional['PROPERTY_LIST_NAME_VALUE'][$i]?>" >
                                                                    <input class="base-name-input-<?=$i?>"  type="hidden" name="USLUGI[BASENAME][]" value="<?=$service_additional['PROPERTY_LIST_NAME_VALUE'][$i]?>">
                                                                    <input type="hidden" name="TYPE" class="quick-filter__select-id-<?=$items[$i]['ID']?>" value="<?=$items[$i]['ID']?>">
                                                                    <input type="hidden" name="TYPE" class="quick-filter__select-price-<?=$items[$i]['ID']?>" value="<?=$items[0]['PROPERTY_LIST_PRICE_VALUE'][$i]?>">
                                                                    <?=$service_additional['PROPERTY_LIST_NAME_VALUE'][$i]?>
                                                                </li>
                                                            <?endfor;?>
                                                        </ul>
                                                    </div>
                                        ...
                                        <?endif;?>
                                        <? if (!empty($service_additional['PROPERTY_SCROLL_NAME_VALUE'])):?>

                                            <div class="details-form__block-title"><?=$service_additional['PROPERTY_SCROLL_NAME_VALUE']?></div>
                                            <div class="details-form__block-inner">
                                                <div class="details-form__range">
                                                    <label class="details-form__range-value">
                                                        ...
                                                        <input class="input-scroll-blank"  name="USLUGI[SCROLL][VALUE]" type="text" data-type="" maxlength="2"
                                                               value="<?=$service_additional['PROPERTY_SCROLL_IN_ONE_VALUE']?>">
                                                    </label>
                                                </div>
                                                ...
                                                <input type="range" min="1" value="1" max="99" class="details-form__range-input" oninput="inputRangeChange()">
                                                <script>
                                    ...
                                    if (!empty($service_additional['PROPERTY_CHECK_NAME_VALUE'])):?>

                                        <? for($i = 0; $i < count($service_additional['PROPERTY_CHECK_NAME_VALUE']);$i++):?>
                                            <div class="details-form__group">
                                                <label class="label-checkbox">
                                                    ...
                                                    <div class="checkbox-text">
                                                        <div class="ckeckbox-title filterSize checkbx-<?=$i?>"><?=$service_additional['PROPERTY_CHECK_NAME_VALUE'][$i]?></div>
                                    ...
                                    if (!empty($service_additional['PROPERTY_DISCOUNT_NAME_VALUE'])):?>
                                        <div >
                                            <div class="details-form__block-title">Скидки:</div>
                                        </div>
                                        <? for($i = 0; $i < count($service_additional['PROPERTY_DISCOUNT_NAME_VALUE']);$i++):?>
                                            <div class="details-form__group">
                                                <label class="label-checkbox">
                                                    ...
                                                    <div class="checkbox-text">
                                                        ...
                                                        <div class="ckeckbox-title"><?=$service_additional['PROPERTY_DISCOUNT_NAME_VALUE'][$i]?></div>
                                                        <div
                                    ...
                                    if ($service_additional['ID'] == '445' ):?>
                                        <div class="details-form__price details-form__total">
                                            Итого:
                                            <div class="details-form__price-value details-form__total-value"><span  data-base="<?=$basePrice?>" data-current="0" data-discount="<?=count($service_additional['PROPERTY_DISCOUNT_NAME_VALUE'])?>"
                                    ...
                                    <?else:?>
                                     ...
                                     <?php else:?>
                                         data-count="<?=count($service_additional['PROPERTY_NAME1_TANK_VALUE'])?>"
                                     <?php endif;?>
                                     class="do-ajax"><?=number_format($basePrice, 0, '', ' ')?></span> ₽</div>
                                        </div>
                                    <?endif;?>
                                    ...
                                    <?}
                                        ...
                                        <div class="form-content">
                                            <div class="details-form__tasks">
                                                <div class="details-form__tasks-title">Поможем выполнить личные боевые задачи (ЛБЗ) на&nbsp;танк&nbsp;<?=$service['PROPERTY_NAME_VALUE'];?></div>
                                                <a href="#" class="details-form__tasks-btn btn tasks-modal-up">Выбрать задачу</a>
                                            </div>
                                        ...
                                        <div class="details-form__price details-form__total">
                                            ...
                                            <div class="details-form__price-value details-form__total-value"><span  data-base="<?=$basePrice?>" data-current="0"
                                             <?php if(count($service_additional['PROPERTY_CHECK_NAME_VALUE']) > count($service_additional['PROPERTY_NAME1_TANK_VALUE'])):?>
                                                 ...
                                                 count-lbz="<?=count($service_additional['PROPERTY_CHECK_NAME_VALUE'])?>"
                                             <?php else:?>
                                                 data-count="<?=count($service_additional['PROPERTY_NAME1_TANK_VALUE'])?>"
                                                 count-parent="<?=$lbzCount?>"
                                                 count-lbz="<?=count($service_additional['PROPERTY_NAME1_TANK_VALUE'])?>"
                                             <?php endif;?>
                                ...
                                <div class="details-form__btns">
                                    ...
                                    <button  class="details-form__btn btn">Cделать заказ</button>
                                </div>
                            </div>
                            <?php if(count($service_additional['PROPERTY_CHECK_NAME_VALUE']) > count($service_additional['PROPERTY_NAME1_TANK_VALUE'])):?>
                                <input type="hidden" name="DEV[LBZ_COUNT]" value="<?=count($service_additional['PROPERTY_CHECK_NAME_VALUE'])?>">
                            <?php else:?>
                                <input type="hidden" class="lbz-count" name="DEV[LBZ_COUNT]" value="<?=count($service_additional['PROPERTY_NAME1_TANK_VALUE'])?>">
                            <?php endif;?>
                            <input class="lbz-parent-count" type="hidden" name="DEV[LBZ_PARENT_COUNT]" value="<?=$lbzCount?>">

                        </form>
                        <div class="details-text">
                            <div class="details-title title"><?=$service['PROPERTY_NAME_VALUE']?></div>

                            <div class="details-links">
                                <a href="/otzyvy/" class="details-link link">Отзывы</a>
                                <button  id="question" class="details-link link">Частые вопросы</button>
                            </div>
                            ...
                            $currentSection = CIBlockSection::GetList(array(), Array('IBLOCK_ID' => 12, 'ACTIVE' => 'Y', 'DEPTH_LEVEL' => 3, 'SECTION_ID' => $currentIDRightOutput), false,
                                array('ID', 'NAME', 'UF_ENGLISH_NAME'),
                                false);

                            while($section = $currentSection->GetNext()):?>
                                <div class="details-subtitle"><?=$section['NAME']?></div>
                            <?
                            $service_detail = CIBlockElement::GetList(
                                array(),
                                ...
                                false,
                                array(),
                                array('ID', 'CODE', 'PROPERTY_SERVICE_INCLUDES', 'PROPERTY_DESCRIPTION', 'PROPERTY_PRICE'));
                            ...
                            foreach ($items1 AS $service_additional):
                                ...
                                <div class="details-text__group">
                                    <div class="details-text__group-title">
                                        ...
                                        <?=$service_additional['PROPERTY_SERVICE_INCLUDES_VALUE']?>
                                    </div>
                                    <ul class="details-text__list">
                                        <? foreach ( $service_additional['PROPERTY_DESCRIPTION_VALUE'] AS $service_d):?>
        ...
        <section class="services other-services">
            <div class="container">
                <div class="services-heading">
                    <div class="services-title title">С этим покупают</div>
                        ...
                        <div class="swiper-wrapper services-slider-wrapper">
                            <?while ($service1 = $services->GetNext()):?>
                                <div    class="swiper-slide services-slider__item">
                                    <a href="/uslugi/<?=$service1['CODE']?>" class="services-item border-b">
                                        <div class="services-item__img border">
                                            <img src="<?= CFile::GetPath($service1['PROPERTY_FOTO_VALUE']) ?>" alt="">
                                        </div>
                                        <div class="services-item__text">
                                            <div class="services-item__title"><?=$service1['PROPERTY_NAME_VALUE']?></div>
                                            <div class="services-item__description">
                                                Процент побед: от <span><?=$service1['PROPERTY_PROCENT_VALUE']?><sub>%</sub></span>
                                            </div>
                                            <div class="services-item__info">
                                                от <span><?=$service1['PROPERTY_CENA_VALUE']?><sup>₽</sup></span> / за бой
                                            </div>
                                        </div>
                                    </a>
                                </div>
            ...
            <div class="popup-wrap">
                <div class="popup-content">
                    ...
                    <form action="#" class="popup-tasks__form form border border-b detail-usluga-ajax">
                        ...
                        <div class="popup-tasks__title">Выберите нужные задачи:</div>
                        <div class="popup-tasks__content">
                            <?php

                            for($i = 0; $i < count($service_additional['PROPERTY_TANK_LIST_VALUE']);$i++): ?>
                                <?php
                                ...
                                <div class="popup-tasks__group">
                                    <div class="popup-tasks__group-title"><?=$service_additional['PROPERTY_TANK_LIST_VALUE'][$i]?></div>
                                    ...
                                    <ul class="popup-tasks__group-list">
                                        ...
                                        <?for($b = 0; $b < count($tank_name[0]);$b++):
                                            if( (preg_match( '/без отл\./i', $tank_name[$i][$b]) || preg_match('/Без отл\./i', $tank_name[$i][$b]) ) || preg_match('/без отличи/i', $tank_name[$i][$b]) || preg_match('/Без отличи/i', $tank_name[$i][$b]) || preg_match('/без отл\./i', $tank_name[$i][$b]) ):?>
                                                <li>
                                                    <label class="label-checkbox">
                                                        ...
                                                        <input type="hidden"   class="visually-hidden lbz-ungreat-<?=$i?> lbz check-ungreat-name" name="LBZ[CHECK_UNGREAT_NAME_<?=$i?>]" value="<?=$tank_name[$i][$b]?>">
                                                        <span></span>
                                                        <div class="checkbox-text">
                                            ...
                                            <?php elseif( preg_match('/отл\./i', $tank_name[$i][$b]) || preg_match('/отличи/i', $tank_name[$i][$b])  ):?>
                                                <li>
                                                    <label class="label-checkbox">
                                                        <input type="checkbox" current-name="<?=$i?>" class="visually-hidden lbz-great-<?=$i?> lbz check-great-price" name="LBZ[CHECK_GREAT_PRICE_<?=$i?>]" value="<?=$tank_price[$i][$b]?>">
                                                        <input type="hidden"   class="visually-hidden lbz-great-<?=$i?> lbz check-great-name" name="LBZ[CHECK_GREAT_NAME_<?=$i?>]" value="<?=$tank_name[$i][$b]?>">
                                                        <span></span>
                                                        <div class="checkbox-text">
                                                            <div class="ckeckbox-title usluga-parent-<?=$i?> lbz-name-<?=$b?>"><?=$tank_name[$i][$b]?></div>
                                                            <div class="ckeckbox-description"><span><?=$tank_price[$i][$b]?></span> ₽</div>
                                            ...
                                            <?php else:?>
                                                <li>
                                                    <label class="label-checkbox" first-id="<?=$i?>" second-id="<?=$b?>">
                                                        ...
                                                        <input type="hidden"   current-id="<?=$i?>" class="visually-hidden lbz-<?=$i?>-<?=$b?> lbz check-name" name="LBZ[CHECK_NAME_<?=$i?>_<?=$b?>]" value="<?=$tank_name[$i][$b]?>">
                                                            <span></span>
                                                            <div class="checkbox-text">
                                                                <div class="ckeckbox-title usluga-parent-<?=$i?> lbz-name-<?=$b?>"><?=$tank_name[$i][$b]?></div>
                                                                <div class="ckeckbox-description"><span><?=$tank_price[$i][$b]?></span> ₽</div>
                                        ...
                                        <?php if($b == count($tank_name[0])-1):?>
                                            <li >
                                                <label class="label-checkbox">
                                                    <input type="checkbox" current-name="<?=$i?>" first-id="<?=$i?>" second-id="<?=$b?>" class="all-tasks-js all-tasks-input-<?=$i?> visually-hidden lbz-all-<?=$i?> lbz check-all-price" name="LBZ[CHECK_ALL_PRICE_<?=$i?>]" value="+<?=$currentSum?>">
                                                    <input type="hidden"   class="visually-hidden lbz-all-<?=$i?> lbz check-all-name" name="LBZ[CHECK_ALL_NAME_<?=$i?>]" value="Все задачи <?=trim($tankListName)?>">
                                                    <span></span>
                                                    <div class="checkbox-text all-tasks-js" first-id="<?=$i?>" second-id="<?=$b?>">
                                                        <div class="ckeckbox-title usluga-parent-<?=$i?> lbz-name-<?=$b?>">Все <?=trim($tankListName)?></div>
                        ...
                        <div class="popup-tasks__btns">
                            <a href="#" class="popup-tasks__btn btn">Сохранить изменения</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        $questions = CIBlockElement::GetList(
            ...
            false,
            false,
            array('ID', 'PROPERTY_QUESTION', 'PROPERTY_ANSWERS'));
        ?>

        <section  class="faq ">
            <div  class="container">
                <div  class="faq-title title">Частые вопросы</div>
                <div class="faq-content">
                    <div class="faq-items">
                        <?while ($question = $questions->GetNext()):?>
                            <div class="faq-item border">
                                <div class="faq-item__heading">
                                    <div class="faq-item__title"><?=$question['PROPERTY_QUESTION_VALUE']?></div>
                                    <div class="faq-item__img">
                                        <img src="<?= SITE_TEMPLATE_PATH ?>/img/faq-arrow.svg" alt="">
                                    </div>
                                </div>
                                <ul class="faq-item__dropdown">
                                    <?foreach ($question['PROPERTY_ANSWERS_VALUE'] AS $item):?>
                                        <li><?=$item['TEXT']?></li>
                                    <?endforeach;?>
...
<script>
    $('.quick-filter__tank').click(function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/ajax/detail/detail.php?id='+params,
            data: {"number": id},
            dataType: 'html',
